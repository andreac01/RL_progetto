...
architecture arch_project_reti_logiche of project_reti_logiche is
    ...
    type STATE is (WAIT_START, GET_CHANNEL, GET_ADDR, WRITE_OUT, ENABLE_OUT);
...
begin
    ...
    channel_calc : process(we_ch, i_w, i_clk)
    -- Processo sequenziale
    begin
        if i_clk'event and i_clk='1' and we_ch='1' then
    ...
    mem_addr_calc : process(we_addr, clr_addr, i_w, i_clk)
    -- Processo sequenziale
    begin
        if i_clk'event and i_clk='1' then
            if clr_addr='1' then
                reg_addr <= "0000000000000000";
            elsif we_addr = '1' then
    ...
    reg_z0_calc : process(i_clk, i_rst, we_z0)
    -- Processo sequenziale
    begin
        if i_rst = '1' then
            reg_z0 <= "00000000";
     ...
     -- Registro di memoria per salvare l'ultimo valore utile sul canale
    reg_z1_calc : process(i_clk, i_rst, we_z1)
    -- Processo sequenziale
    begin
        if i_rst = '1' then
            reg_z1 <= "00000000";
    ...
    -- Registro di memoria per salvare l'ultimo valore utile sul canale
    reg_z2_calc : process(i_clk, i_rst, we_z2)
    -- Processo sequenziale
    begin
        if i_rst = '1' then
            reg_z2 <= "00000000";
    ...
    -- Registro di memoria per salvare l'ultimo valore utile sul canale
    reg_z3_calc : process(i_clk, i_rst, we_z3)
    -- Processo sequenziale
    begin
        if i_rst = '1' then
            reg_z3 <= "00000000";
    ...
    o_z : process(reg_z0, reg_z1, reg_z2, reg_z3, out_en)
    -- Processo combinatorio
    begin
        if out_en = '0' then
            o_z0 <= "00000000";
